							...
							<div class="table-toolbar">
							 ...
							 <table class="table table-striped table-hover table-bordered" id="sample_editable_1">
								...
								@foreach($shop as $value)
									...
									<tr id="{{$value->id}}">
										<td>{{$value->id}}</td>
										<td>{{$value->shopfood_name}}</td>
										<td>{{$value->shopfood_slug}}</td>
                                        <td>{{$value->shopfood_address}}</td>
										<td class="center">
											<div class="toggle-button-demo">
											  <input name="activecp" class="toggle toggle-ios" id="toggle{{$value->id}}" @if($value->shopfood_active == 0) checked @endif type="checkbox" value=""/>
											  <label class="toggle-btn" for="toggle{{$value->id}}"><span>On</span></label>
											</div>
										</td>
										<td class="center" style="text-align: center;">
											<a id="{{$value->id}}" class="deleteShop">
